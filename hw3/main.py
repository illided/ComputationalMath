...
def task_3_1():
    ...
    st.write(f"Используемая функция: {function}")
...
def task3_2():
    ...
    frame = pd.DataFrame([X, Y, predicted_fd, fd_error, predicted_sd, sd_error],
                              index=["x", "f(x)", "f'(x)", "f'(x) error", "f''(x)", "f''(x) error"]).T
    st.write(frame.style.format("{:.10}"))
